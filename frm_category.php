...
<body class="easyui-layout" style="width:100%;height:100%">
	...
	<table id="dg" title="Data Category" class="easyui-datagrid" fit="true"
			url="query/view_category.php"
			toolbar="#toolbar" pagination="true"
			rownumbers="true" fitColumns="true" singleSelect="true">
		<thead>
			<tr>
				<th field="name" width="100" sortable="true">Category Name</th>
	...
	<div id="toolbar">
		...
		<a href="javascript:void(0)" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="deleteCategory()">Delete Category</a>		
	</div>
	...
	<script type="text/javascript">
		...
		function newCategory(){
			...
			$('#fm').form('clear');
			//alert('query/action_category.php?action=insert');
			url = 'query/action_category.php?action=insert';
		}
		...
		function deleteCategory(){
			...
			if (row3){
				$.messager.confirm('Confirm','Are you sure you want to delete this data?',function(r){
					if (r){
						$.post('query/action_category.php',{category_id:row3.category_id, action:'delete'},function(result){							
							if (result.success){
		...
		/*
			...
			if (row){
				$.messager.confirm('Confirm','Are you sure you want to delete this data?',function(r){
					if (r){
						$.post('query/action_category.php',{category_id: row.category_id, action: 'delete'},function(result){
							if (result.success){
								$('#dg').datagrid('reload');	// reload the user data
							} else {
								$.messager.show({	// show error message
